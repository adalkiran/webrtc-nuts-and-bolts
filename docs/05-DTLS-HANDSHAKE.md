...
If this buffer part comply with these conditions, we can say "this packet is a DTLS protocol packet", then we can process it with DTLS protocol's methods.
...
* In the screenshoot below (and also the screenshoot at chapter 5.9), we can see the concatenation result took *1179 bytes*.
...
<br>
So we need (16+16+4+4) = 40 bytes which generated by PHash with our masterSecret and seed. Then we extract our key and IV values from this 40 bytes sequentally.
